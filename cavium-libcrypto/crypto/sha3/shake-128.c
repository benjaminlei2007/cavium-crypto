...
int SHAKE_128_init(SHAKE_CTX* ctx)
{
    uint32_t capacity = SHAKE_128_SECURITY_STRENGTH*2;
    uint32_t rate = SnP_width - capacity;
    int result = Keccak_spongeinitialization(&ctx->sponge,rate,capacity);
    if (result != 0)
        return result;
    ctx->fixedOutputLength = SHAKE_128_BITLEN;
...
int SHAKE_128_Update(SHAKE_CTX* ctx,uint8_t* data,uint32_t size)
{
    if(size % 8 == 0)
    ...
    else
    {
        ...
        if (ret == 0) {
            ...
            if ((delimitedLastBytes & 0xFF00) == 0x0000) {
                ...
                ret = (int)Keccak_SpongeAbsorb(&ctx->sponge, oneByte, 1);
...
int SHAKE_128_Final(SHAKE_CTX* ctx, uint8_t* hash, 
...
/* Pass squeezedoutputlength in bit format */
    ret = shake_128_HashSqueeze(ctx, hash, squeezedoutputlength*8);
    if(ret < 0)
        return ret;

    return 0;
}
int shake_128_HashSqueeze(SHAKE_CTX *ctx, uint8_t *data, 
        uint32_t databitlen)
{
    if ((databitlen % 8) != 0)
        return -1;
