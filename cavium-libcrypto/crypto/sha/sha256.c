...
#ifdef OCTEON_OPENSSL
  sha_assert (c, -1);
  c->iv[0] = 0xc1059ed8367cd507ull;
  c->iv[1] = 0x3070dd17f70e5939ull;
  c->iv[2] = 0xffc00b3168581511ull;
  c->iv[3] = 0x64f98fa7befa4fa4ull;
  c->totalin = 0;
  c->done = 0xbabe;
  c->plen = 0;
  c->sha256 = 0;
...
#ifdef OCTEON_OPENSSL
  sha_assert (c, -1);
  c->iv[0] = 0x6a09e667bb67ae85ull;
  c->iv[1] = 0x3c6ef372a54ff53aull;
  c->iv[2] = 0x510e527f9b05688cull;
  c->iv[3] = 0x1f83d9ab5be0cd19ull;
  c->totalin = 0;
  c->done = 0xbabe;
  c->plen = 0;
  c->sha256 = 1;
...
unsigned char *SHA224(const unsigned char *d, size_t n, unsigned char *md)
	...
	octeon_sha224((uint64_t *)d, n, (uint64_t *)md);
	return(md);
	}
...
unsigned char *SHA256(const unsigned char *d, size_t n, unsigned char *md)
	...
	octeon_sha256((uint64_t *)d, n, (uint64_t *)md);
	return(md);
	}
...
SHA256_Update (SHA256_CTX * ctx, const void *data, size_t len)
{
  ...
  if ((ctx->plen + len) >= 64) {
    ...
    if (ctx->plen) { 
      uint64_t *ptr = (uint64_t *) ctx->pbuf;
      memcpy (&ctx->pbuf[ctx->plen], data, 64 - ctx->plen);
      len -= (64 - ctx->plen);
      data = (uint8_t *) data + (64 - ctx->plen);
      ctx->plen = 0;
    ...
    memcpy (&ctx->pbuf[ctx->plen], data, len);
    ctx->plen += len;
  }
...
SHA256_Final (unsigned char *md, SHA256_CTX * ctx)
{
  ...
  if (ctx->done == 0xbabe) {    
    ...
    ctx->pbuf[ctx->plen] = 0x80;
    memset (ctx->pbuf + ctx->plen + 1, 0, 64 - ctx->plen - 1);
    ...
    CVMX_MT_HSH_DAT (*ptr++, 6);
    if (ctx->plen < 56) {
      CVMX_MT_HSH_STARTSHA256 (ctx->totalin * 8);
    } else {
      CVMX_MT_HSH_STARTSHA256 (*ptr);
